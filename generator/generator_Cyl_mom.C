void generator_Cyl_mom(double zmax, double r2max, double momentum, string detector, int particle, int neutrino, double mass){
  //zmax: half of detector height
  //r2max: detector radius
  //momentum: momentum of target particle
  ...
  r2max=r2max-200;
    ...
    r2max*=r2max;
    sprintf(filename,"eventlist_%s_particle%d_%dMeVpc_mom.txt",detector.c_str(),particle,(int)momentum);
	...
	double energy = sqrt(momentum*momentum+mass*mass)-mass;
